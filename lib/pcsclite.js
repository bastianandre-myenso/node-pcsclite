...
// Load the correct binding for the current platform
let pcsclite = require('../build/Release/pcsclite.node');
const platform = os.platform(); // 'win32', 'linux', 'darwin'
try {
	if (platform === 'win32') {
		pcsclite = require('../prebuilt/windows/pcsclite.node');
    } else if (platform === 'linux') {
		pcsclite = require('../prebuilt/linux/pcsclite.node');
    }
} catch (error) {
	console.error('Error loading prebuilt pcsclite bindings. Using default.');
}
